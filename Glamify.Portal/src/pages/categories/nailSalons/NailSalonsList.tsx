...
export const NailSalonsList: React.FC = () => {
  ...
  const [selectedNailSalon, setSelectedNailSalon] = useState<Business | null>(null)
  ...
  const fetchBusinesses = async () => {
    setLoading(true)
    try {
      const data = await businessService.getBusinessesOfType(BusinessType.NailSalon)
      setData(data)
    } catch (error) {
  ...
  const handleBooking = async (appointmentTime: string) => {
    if (!user || !selectedNailSalon || !selectedService) {
      console.error("User, nail salon, or service data is missing")
      return
    }
    ...
    const newAppointment: CreateAppointmentRequest = {
      userId: user.id,
      user: user,
      locationId: selectedNailSalon.id,
      location: selectedNailSalon,
  ...
  const openBookingModal = (nailSalon: Business, service: Service) => {
    setSelectedNailSalon(nailSalon)
    setSelectedService(service)
    setShowModal(true)
  }

  const closeBookingModal = () => {
    setSelectedNailSalon(null)
        ...
        {data.map((nailSalon) => (
          <Row key={nailSalon.id} className="mb-3">
            ...
            <Col md={9} className="text-md-right">
              {services
                .filter((x) => x.businessId === nailSalon.id)
                .map((service) => (
                  <>
                    <div key={service.id} className="flex-container">
                      ...
                      <div className="flex-item details">
                        ...
                        <Button
                          variant="primary"
                          onClick={() => openBookingModal(nailSalon, service)}
                          className="book-btn"
                        >
                          Book
                        </Button>
